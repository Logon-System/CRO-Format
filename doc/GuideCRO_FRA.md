...
Le format CPR a été construit à partir d'une méthode standard appelée "RIFF". Il est cependant trop simpliste car les roms ne possèdent aucune propriété et doivent être présentes dans le fichier dans l'ordre physique ou elles seront traitées. Et ceci se révèle problématique.  

En effet, les machines PLUS sont rétro compatibles avec l'ancienne gamme, sur laquelle il existe une notion de rom logique. C'est notamment le cas avec la rom disque (AMSDOS) qui possède le numéro logique 7, bien que la machine ne dispose nue que de 3 roms au maximum (6128 et 664 pour les anciennes génération). Lorsqu'un programme sur l'ancienne gamme sélectionne la rom 7, cette dernière sait électroniquement qu'elle est sollicitée et répond présent. Son "ordre physique" importe peu et n'a que peu de sens.  

Afin de conserver cette compatibilité avec l'ancienne gamme, les ingénieurs Amstrad, qui avaient déjà sacrifié le nombre de roms possible de 256 à 32 pour la gamme du Plus, se sont dit qu'ils pouvaient sacrifier la capacité d'accès du nombre de roms classique de l'ancien système émulé pour les cartouche de 256 à 128. Via l'utilisation du bit 7 du numéro de rom (sur le port de sélection &DFFF), cela permet de sélectionner une rom par transposition logique (bit 7=0), ou de sélectionner une rom par son numéro physique (bit 7=1). Les ingénieurs se sont dit que sans doute jamais personne n'utiliserait plus de 128 Roms sur un CPC. Ce qui était vrai dans les années 90 ne l'est sans doute plus aujourd'hui.

Les roms d'une cartouche étant "stockées" au sein d'une même mémoire limitée à 512k, il a fallu déterminer une table de transposition logique pour assurer la compatibilité avec les programmes de l'ancienne gamme accédant à la rom 7. En l'occurrence, pour la rom logique 7, ils ont décrété que la rom physique correspondante serait la numéro 3 dans une cartouche. Autrement dit, sur la gamme des Plus, faire `OUT &DF00,7` revient à faire `OUT &DF00,&80+3`. Si la rom logique n'est pas 7, alors la rom physique est la 1, pour reproduire le comportement de l'ancienne génération, qui sélectionne la 1ère rom haute quelque soit la valeur envoyée sur le port de sélection (à l'exception de la 7). La rom 0 "physique" dans la cartouche est réservée à la rom "basse" du CPC d'ancienne génération. Sur ce dernier point, la gamme des PLUS permet aussi de sélectionner une des 8 premières rom physique de la cartouche comme rom basse (mais également de fixer la base de cette rom ailleurs qu'en &0000 (&4000 ou &8000 via un registre de l'ASIC appelé RMR2)).  

On voit donc ici que le format CPR n'est pas adapté pour tenir compte de ces particularités sur la gamme des Plus ou la transposition d'un numéro logique vers un numéro physique (traduit par les lignes d'adresse d'accès à la rom) impose la présence d'autres roms physiques dans le fichier. Ainsi, lorsque Amstrad a sorti en France une cartouche sans le jeu "Burnin' Rubber" à cause de problème de compatibilité (lié au bidouillage de la rom disque par Amstrad pour y caser le menu de sélection entre le jeu et le basic), ils ont créé une cartouche contenant 4 roms au lieu de 3, puisque la rom 7 est physiquement la 4ème et que la technologie de l'époque orientait le choix technique vers une eeprom linéaire. Ainsi, dans cette cartouche, ainsi que dans le CPR qui en a été extrait, on trouve en position 3 (numéro 2) une rom qui ne sert absolument à rien, sinon d'occuper l'espace.  
