...
The CPR format was built using a standard method called "RIFF".

However, it is simplistic because ROMs have no properties (size, type).

ROMs are all 16k and are encapsulated within a 512k limit.

The PLUS machines are backward compatible with the older range, which uses the concept of a logical ROM. This is particularly true for the disk ROM (AMSDOS), which has the logical number 7, even though the machine only has a maximum of 3 ROMs (6128 and 664 for older generations). When a program on the older range selects ROM 7, the ROM electronically recognizes that it is being used and responds accordingly. Its "physical order" is irrelevant and largely meaningless.

To maintain compatibility with the older system, Amstrad engineers, who had already reduced the number of possible ROMs from 256 to 32 for the Plus series, figured they could reduce the ROM capacity of the older emulated system to 128 cartridges from 256 to 128. By using bit 7 of the ROM number (on the &DFFF selection port), a ROM could be selected by logical transposition (bit 7=0), or by its physical number (bit 7=1). The engineers may have assumed that no one would use more than 128 ROMs on a CPC. What was true in the 1990s is probably no longer true today.

Since the ROMs of a "Plus" cartridge are "stored" within a single memory limited to 512k, a logical transposition table exists to ensure compatibility with programs from the older range accessing ROM 7. Specifically, for logical ROM 7, the corresponding physical ROM is number 3 in a cartridge. In other words, on the Plus range, entering `OUT &DF00,7` is equivalent to entering `OUT &DF00,&80+3`. If the logical ROM is not 7, then the physical ROM is 1, to replicate the behavior of the older generation, which selects the first high ROM regardless of the value sent to the selection port (except for ROM 7). The "physical" ROM 0 in the cartridge is reserved for the "low" ROM of the older generation CPC. On this last point, the PLUS series also allows you to select one of the first 8 physical ROMs on the cartridge as the base ROM (and also to set the base of this ROM to a location other than &0000 (&4000 or &8000 via an ASIC register called RMR2)).

When Amstrad released a cartridge in France without the game "Burnin' Rubber" due to compatibility issues (related to Amstrad's modification of the disk ROM to accommodate the selection menu between the game and BASIC), they created a cartridge containing 4 ROMs instead of 3. This is because ROM 7 is physically the 4th ROM, and the technology of the time favored a linear EEPROM. Thus, in this cartridge, as well as in the CPR extracted from it, there is a ROM in position 3 (number 2) that serves absolutely no purpose (a remnant of the game), other than to occupy space.

The CPR is also not suitable for encapsulating ROMs for older generation CPCs.
...
Read formula, assuming MaskSizeCartridge represents the total size minus 1 of the ROM accessible via the card:
`ReadAddress = ((CpcBusAddress AND GGRO[CurrentGroup].GMSK) + GGRO[CurrentGroup].BaseAddress) and MaskSizeCartridge`

`CurrentGroup` results from a **GROUPSELECT** command passed to the card.
